...
public class SkillFlowTagLayout extends ViewGroup {
     ...
     */
    OnSkillTagClickListener mOnTagClickListener;

    /**
     * the tag select event callback
     */
    OnSkillTagSelectListener mOnTagSelectListener;
    ...
    public SkillFlowTagLayout(Context context) {
        super(context);
    }

    public SkillFlowTagLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public SkillFlowTagLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void reloadData() {
        ...
        for (int i = 0; i < mAdapter.getCount(); i++) {
            ...
            //region childView的OnClickListener
            childView.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (j==0){
                        mOnTagSelectListener.onShowInput("show");
                    } else {
                        //region 对shy没有用的代码
                        if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
                            if (mOnTagClickListener != null) {
                                mOnTagClickListener.onItemClick(SkillFlowTagLayout.this, childView, j);
                            }
                            ...
                            if (mCheckedTagArray.get(j)) {
                                ...
                                if (mOnTagSelectListener != null) {
                                    mOnTagSelectListener.onItemSelect(SkillFlowTagLayout.this, new ArrayList<Integer>());
                                }
                                return;
                            }
                            ...
                            if (mOnTagSelectListener != null) {
                                mOnTagSelectListener.onItemSelect(SkillFlowTagLayout.this, Arrays.asList(j));
                            }
                        }
                        //endregion
                        else if (mTagCheckMode == FLOW_TAG_CHECKED_MULTI) {
...
//                                }
                                map.put(j, isSelect);
                                list.add(map);
                                mOnTagSelectListener.onItemSelects(SkillFlowTagLayout.this, list);
                            }
                        }
                    }
                }
    ...
    public void setOnTagClickListener(OnSkillTagClickListener onTagClickListener) {
        this.mOnTagClickListener = onTagClickListener;
    }

    public void setOnTagSelectListener(OnSkillTagSelectListener onTagSelectListener) {
